...
module.exports.updateComponent = function(component, config, username, email, callback){
    ...
    if(version >= config.latest){
        ...
        Keyword.remove(component.name, username)
            .then(
                function(result){
            ...
            if(version == "all"){
                ...
                async.each(files, fis.db.unlink, function(error){
                    if(error){
                        ...
                        Keyword.remove(config.name, username)
                            .then(
                                function(){
...
//todo 最新版本计算有bug
                            ...
                            Keyword.remove(config.name, username)
                                .then(
                                    function(result){
                                        return Keyword.update(config.name, component.keywords, username);
                                    }
                                )
                                .then(
                                    function(result){},
